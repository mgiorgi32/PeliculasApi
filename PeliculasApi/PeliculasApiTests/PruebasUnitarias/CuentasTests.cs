...
{
    [TestClass]
    public class CuentasTests : BasePruebas
    {
        [TestMethod]
        ...
        [TestMethod]
        public async Task NoPuedeLoguearse()
        {
            var nombreBD = Guid.NewGuid().ToString();
            await CrearUsuarioHelper(nombreBD);
        ...
        [TestMethod]
        public async Task PuedeLoguearse()
        {
            var nombreBD = Guid.NewGuid().ToString();
            await CrearUsuarioHelper(nombreBD);
            var controller = ConstruirCuentasController(nombreBD);
        ...
        private CuentasController ConstruirCuentasController(string nombreDB)
        {
            ...
            var miConfig = new Dictionary<string, string>()
            {
                {"JWT:key", "jahsjdnsdishohaohdhHUHundjnxishUHAUHSUHHWU" }
            };
            var config = new ConfigurationBuilder().AddInMemoryCollection(miConfig).Build();
            return new CuentasController(userManager, signInManager, config, context, mapper);
