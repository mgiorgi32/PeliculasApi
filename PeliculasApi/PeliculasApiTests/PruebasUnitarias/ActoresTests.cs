...
{
    [TestClass]
    public class ActoresTests: BasePruebas
    {
        [TestMethod]
        public async Task ObtenerPersonasPaginadas()
        {
            ...
            var pagina1 = await controller.Get(new PaginacionDto() { Pagina = 1, CanridadRegistrosPorPagina = 2 });
            var actoresPagina1 = pagina1.Value;
            Assert.AreEqual(2, actoresPagina1.Count);
            controller.ControllerContext.HttpContext = new DefaultHttpContext();
            var pagina2 = await controller.Get(new PaginacionDto() { Pagina = 2, CanridadRegistrosPorPagina = 2 });
            var actoresPagina2 = pagina2.Value;
            Assert.AreEqual(1, actoresPagina2.Count);
            controller.ControllerContext.HttpContext = new DefaultHttpContext();
            var pagina3 = await controller.Get(new PaginacionDto() { Pagina = 3, CanridadRegistrosPorPagina = 2 });
        ...
        public async Task CrearActorSinImagen()
        {
            ...
            var contexto = ConstruirContext(nombreBD);
            var mapper = ConfigurarAutoMapper();
            var actor = new ActorCreacionDto() { Nombre = "Marcos", FechaNacimiento = DateTime.Now };
        ...
        [TestMethod]
        public async Task PatchRetorna404SiElActorNoExiste()
        {
            var nombreBD = Guid.NewGuid().ToString();
            var contexto = ConstruirContext(nombreBD);
            var mapper = ConfigurarAutoMapper();
